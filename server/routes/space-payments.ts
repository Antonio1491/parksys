...
export function registerSpacePaymentRoutes(app: Express) {
  ...
  app.post("/api/space-reservations/:id/payment-confirm", async (req, res) => {
    try {
      ...
      try {
        // Obtener el payment intent de Stripe para extraer metadata
        ...
        const costingResponse = await fetch(`${process.env.BASE_URL || 'http://localhost:5000'}/api/costing/process-payment`, {
          ...
          body: JSON.stringify({
            ...
            customerEmail: updatedReservation[0].contactEmail || 'unknown'
          })
        });

        if (costingResponse.ok) {
          const costingResult = await costingResponse.json();
          console.log(`💰 Costeo procesado automáticamente para reserva ${id}:`, costingResult.costingMetrics);
        } else {
          console.warn(`⚠️ No se pudo procesar costeo para reserva ${id}:`, await costingResponse.text());
        }
      } catch (costingError) {
