...
**Static Files System Fix (August 2025)**: Resolved critical issue with static file serving for production deployments. Added Express routes in server/index.ts for `/images/*`, `/uploads/*`, `/fonts/*`, and `/locales/:lang/:namespace.json` endpoints with proper file system checks, MIME type detection, and error handling. These specific routes are required because vercel.json redirects these paths to the Node.js server. Fixed Vercel deployment configuration: Framework must be set to "Other" (not "Vite") with build command `npm run build && node scripts/prepare-vercel-assets.js` and output directory `dist/public`. All static assets now serve with HTTP 200 and proper MIME types both locally and in production.
...
**Public Instructor Evaluation Form Fix (August 2025)**: Resolved critical 400 error in public instructor evaluation submission endpoint. Fixed insertInstructorEvaluationSchema by adding instructorId to omit() fields since it comes from URL parameters, not request body. Corrected SQL column mapping to match actual database structure (knowledge, communication, methodology, overall_performance). The public evaluation form now works correctly with proper spam prevention (24-hour cooldown per IP), data validation, and pending status for administrative moderation. Verified with successful test submissions creating evaluation records in database.
...
**Activities Manual Implementation (August 2025)**: Successfully developed and integrated comprehensive Activities manual into ParkSys Help Center. Created complete manual with 9 detailed sections covering all aspects of the activities module: introduction, activity listing and management, categories system, instructor management, registration system, scheduling and calendars, reports and analytics, best practices, FAQs, and technical support. Manual integrated into main Help Center as "Manual Completo - Gestión de Actividades" (without "Eventos" as requested), accessible through DocumentationViewer component with proper navigation and search capabilities. Route added to App.tsx with lazy loading for optimal performance.
